...
my $NWORDS = train(words(slurp_file('big.txt')));
...
sub correct
{
    ...
    my @canidates;
    push @canidates, $word;
    push @canidates, known($word);
    push @canidates, known(edits1($word));
    push @canidates, known(edits2($word));
...
my $tests2 = {'forbidden'=> 'forbiden', 'decisions'=> 'deciscions descisions',
